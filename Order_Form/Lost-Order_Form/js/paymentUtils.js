var PaymentUtils = function() {
    ...
    this.getSubscriptionPeriodConfig = function(unit, interval) {
        return SUBSCRIPTION_PERIOD_CONFIG.find(({
            interval: _interval,
            unit: _unit
        }) => (_interval === interval && _unit === unit));
    };
    ...
    this.getPaymentSummaryForCheckout = function() {
        ...
        selectedProducts.forEach(function(product) {
            ...
            if (productQty.length === 1) { // if product has no subproduct
                ...
                productQty.forEach(function(qty) {
                    quantity += qty.quantity
                }) // calculating quantity for subproduct
    ...
    this.getTaxSurchargeData = function() {
        if (!JotForm.surchargeFieldId) {
            return {};
        }

        const qid = JotForm.surchargeFieldId;
        const inputs = document.querySelectorAll(`#id_${qid} input, #id_${qid} select`);
        const val = Array.from(inputs).reduce((inputs, currentInput) => {
            return {
                ...inputs,
                [currentInput.getAttribute('data-component')]: currentInput.value
            }
        }, {});
        return {
            [qid]: val
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (productInput && productInput.checked && !items[i].classList.contains('sold_out_product')) {
                if (productInput.classList.contains('form-product-has-subproducts')) {
                    ...
                    if (_customOptionValues.length === 0) {
                        _customOptionValues.push('1');
                    }
                    ...
                    if (specialProductInputs.length === 0) {
                        ...
                        if (!isExpandedProduct) {
                            ...
                            if (Object.entries(groupedData).length > 0) {
                                selectedProducts = [...selectedProducts, {
                                    [pid]: Object.values(reformatGroupedData(groupedData))
                                }];
                            }
                        }
                    }
    ...
    this.getSelectedSubscriptionCustomData = function() {
        ...
        Array.from(customInputs).forEach(input => {
            if (input.id.includes('_custom_first_payment_price')) {
                subscription[pid][0].firstPayment = input.value;
            }
            if (input.id.includes('_custom_price')) {
                subscription[pid][0].price = input.value;
            }
        });
    ...
    this.getSubscriptionPriceSummary = function() {
        ...
        if (!selectedSubscriptionPid) {
            return {};
        }
        const {
            firstPayment: customFirst,
            price: customRecur
        } = customData[0][selectedSubscriptionPid][0];
    ...
    this.attachPaymentSummaryToForm = function() {
        ...
        if (!!JotForm.donationField) {
            form.insert(new Element('input', {
                type: 'hidden',
                name: 'paymentSummary'
            }).putValue(JSON.stringify({
                total: JotForm.donationField.value
            })));
        }
        if (JotForm.pricingInformations && JotForm.pricingInformations.general) {
            ...
            const paymentSummary = {
                ...
                ...(window.paymentType === 'product' && {
                    discount: JotForm.morePreciseToFixed(shippingDiscountValue + taxDiscountValue + parseFloat(discount)) || 0
                }),
                ...(window.paymentType === 'subscription' && {
                    ...this.getSubscriptionPriceSummary()
                })
            };
            form.insert(new Element('input', {
                type: 'hidden',
    ...
    this.getDonationItem = function(gateway = '') {
        ...
        return {
            ...
            quantity: 1,
            unit_amount: {
                value: parseFloat(JotForm.paymentTotal),
                currency_code: self.getCurrency(gateway)
            }
        };
    };
    ...
    this.observeFormSubmitAction = function(elements, hasCreditCardSet, input, validatePaymentMethodFieldSelection, callback) {
        ...
        this.getForm().addEventListener('submit', function paymentUtils(event) {
            ...
            if (isValidated && validatePaymentMethodFieldSelection() === 'paypalSPB' && !window.userInitializedFastlane) {
                /** because we don't expect to see paypal spb payment method at this point.
                 	replicable only if control_paymentmethod is hidden by conditions etc
                 	paypal spb option hides submit button and render its own buttons and then continue for submitting the form after creating order
                 	in this case, submit button stays on the form and payment method will not be shown although paypal spb is required
                 */
                isValidated = false;
            }
            // If the coupon has been applied and affected 100% and the amount is 0 then exit this form validation. 
            ...
            if (isValidated) {
                ...
                if (errors.length > 0) {
                    if (self.getPaymentMethodField()) {
                        ...
                        var isPaymentMethodFieldVisible = JotForm.isVisible(paymentMethodField);
                        if (paymentMethodField && isPaymentMethodFieldVisible && isPaymentMethodSectionTouched && self.dummyInput.length > 0) {
                            JotForm.errored(self.dummyInput[0], errors);
                        }
